...
####################################################
#  Class for working with a meter Класс для работы с измерителем AC (Hopi HP-9800)
####################################################
class acmeter(object):
    ...
    #####################################################################
    #   Serial port initialization, set at the beginning of the program Инициализация последовательного порта, задаётся в начале программы
    #####################################################################
    def __init__(self, port):

        # Trying to open a serial port Пытаемся открыть последовательный порт
        try:
            self.acmeter_serial = serial.Serial(port, baudrate = 9600, timeout = 1)
            print("INFO: Controlled socket connected via serial port Подключена управляемая розетка через последовательный порт {0} \n\r".format(port))
        
        except: # SerialException:
            print("ERROR: Managed socket connection problem: Can't open port Managed socket connection problem: Can't open port Проблема c подключением управляемой розетки: не могу открыть порт Проблема c подключением управляемой розетки: не могу открыть порт {0} \n\r".format(port))
            exit(1)


    #####################################################################
    #  Read the specified register Читаем указанный регистр 
    #####################################################################
    def readRegisters(self, first, count, address = 1):
        ...
        # We pack the bit mass for sending and add Пакуем битовую массу для отправки и добавляем CRC
        measure_request = struct.pack(">BBHH", address, cmd, first, count)
        measure_request += self.crc(measure_request)

        # Sending a request to the device Отправляем запрос к устройству
        ...
        if len(reply) != replyLen:
            print("ERROR: The expected size of the response from the managed outlet does not match the size received. Expected: Ожидаемый размер ответа от управляемой розетки не совпадает с полученным. Ожидаемый:", replyLen, "Actual Фактический: ", len(reply))
            return
        ...
        if reply_crc != reply[-2:]:
            print("ERROR: CRC checksum error from controlled socket response Ошибка контрольной суммы CRC от ответа управляемой розетки ")
            return
        
        # Unpacking the response into variables Распаковываем ответ в переменные
...
return value[1]
    ...
    #####################################################################
    #  Checksum calculation function Функция вычисления контрольной суммы
    #####################################################################
    def crc(self, data):
